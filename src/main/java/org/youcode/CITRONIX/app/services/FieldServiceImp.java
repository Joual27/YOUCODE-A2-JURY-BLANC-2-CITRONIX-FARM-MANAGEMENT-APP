...
import org.youcode.CITRONIX.app.exceptions.FarmFieldsLimitReachedException;
import org.youcode.CITRONIX.app.exceptions.FarmSurfaceExceededException;
import org.youcode.CITRONIX.app.exceptions.FieldMaxSurfaceExceededException;
...
public class FieldServiceImp implements FieldService {
    ...
    public FieldResponseDTO save(CreateFieldDTO data){
        ...
        if (data.surface() > f.getSurface() / 2){
            throw new FieldMaxSurfaceExceededException("The field surface cant be more than half of the farm's surface which is :" + f.getSurface() + "Max value accepted is " +  f.getSurface()/2);
        }
        if (calculateNumberOfFieldsPerFarm(f) == 10){
            throw new FarmFieldsLimitReachedException("This farm already has 10 fields which is the limit ! ");
        }
        if (data.surface() > getOverallAvailableSpaceInFarm(f)){
            throw new FarmSurfaceExceededException("U have exceeded the overall surface of the farm :" + f.getName());
        }
    ...
    public FieldResponseDTO update(UpdateFieldDTO data , Long id){
        ...
        if (data.surface() > getOverallAvailableSpaceInFarm(f.getFarm())){
            throw new FarmSurfaceExceededException("U have exceeded the overall surface of the farm :" + f.getFarm().getName());
        }
    ...
    public Field getFieldEntityById(Long id){
        return fieldPersistenceAdapter.findById(id)
                .orElseThrow(() -> new EntityNotFoundException("No field found with given ID !"));
    }
}
